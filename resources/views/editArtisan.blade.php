<x-app-layout>
    ...
    <div class="box-border p-4 bg-white h-128 w-96">
        <form method="POST" action="{{ route('updateArtisan', $artisan) }}" enctype="multipart/form-data">
        @method('PUT')
                @csrf
            <div class="flex flex-col my-4 text-xl greenAmaso">
                <label for="nombre" class="font-serif">{{ __('Nombre o tu Marca Personal') }}</label>
                <input type="text" id="name" class="w-100 border-solid border-2 border-green-500 rounded shadow-md h-10" name="name" value="{{$artisan->name}}" required autocomplete="name" autofocus>
            </div>
            <div class="flex flex-col my-4 text-xl greenAmaso">
                <label for="imagen" class="font-serif">{{ __('Imagen de Perfil') }}</label>
                <input accept="image/*" type="file" id="image" class="custom-file-input w-100 border-solid border-2 border-green-500 rounded shadow-md h-10"value="{{$artisan->image}}" name="image" autofocus>
            </div>
            <div class="flex flex-col my-4 text-xl greenAmaso">
                <label for="location" class="font-serif">{{ __('Localidad') }}</label>
                <input type="text" id="location" class="w-100 border-solid border-2 border-green-500 rounded shadow-md h-10" name="location" value="{{$artisan->location}}" required autocomplete="location" autofocus>
            </div>
            <div class="flex flex-col my-4 text-xl greenAmaso">
