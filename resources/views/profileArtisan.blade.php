<x-app-layout>
    <body>
        ...
        <button class= "greenLightBg flex flex-row align-start text-sm text-white mt-4 px-6 py-2  rounded-xl shadow-md">               
            ...
            <a href="{{route('editProfile', ['id' => $artisan->id]) }}" method="get"}}>
                Editar perfil
            </a>             
        </button>
        <form method="POST" action="{{ route('deleteProfile', $artisan->slug) }}">
            <button class= "greenLightBg flex flex-row align-start text-sm text-white mt-4 px-6 py-2  rounded-xl shadow-md"type="submit" onclick="return confirm('Esta acción es irreversible, seguro que quieres eliminar tu perfil?')">Eliminar Perfil</button>
            @method('DELETE')
            {{ csrf_field() }}         
        </form>
        @endauth
